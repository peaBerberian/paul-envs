...
- `dotfiles` is now in a project-specific directory alongside `build.conf` and `run.conf`, it can still be initially "seeded" by the config's dotfile
...
- Add `--projects` flag to `clean` command
